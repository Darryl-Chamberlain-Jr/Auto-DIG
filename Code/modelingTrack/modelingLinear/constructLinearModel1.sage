...
displayProblem = "\\begin{center} \\textit{A bicyclist is training for a race on a hilly path. Their bike keeps track of their speed at any time, but not the distance traveled. Their speed traveling up a hill is %d mph, %d mph when traveling down a hill, and %d mph when traveling along a flat portion.} \\end{center}" %(speedUp, speedDown, speedFlat)
