...
\end{center}
The solution is \( k = 1.50 \), which is option D.\begin{enumerate}[label=\Alph*.]
\item \( k = 0.04 \)
...
\begin{center}
    \textit{ Pepsi wants to increase the volume of soda in their cans. They've decided to decrease the radius by 18 percent and decrease the height by 14 percent. They want to model the new volume based on the radius and height of the original cans. }
\end{center}
The solution is \( k = 1.81667 \), which is option D.\begin{enumerate}[label=\Alph*.]
\item \( k = 0.00454 \)

This corresponds to the model: $V = (0.18 r)^2 (0.14 h)$.
\item \( k = 0.01425 \)

This corresponds to the model: $V = \pi (0.18 r)^2 (0.14 h)$.
\item \( k = 0.57826 \)

This corresponds to the model: $V = (0.82 r)^2 (0.86 h)$.
\item \( k = 1.81667 \)

* This is the correct option and corresponds to the model: $V = \pi (0.82 r)^2 (0.86 h)$.
\item \( \text{None of the above.} \)
...
\textbf{Pop} &79880 &79640 &78920 &76760 &70280 &50840 &0 &0 &0\end{tabular}The solution is \( \text{Exponential} \), which is option D.\begin{enumerate}[label=\Alph*.]
...
\begin{center}
    \textit{ Pepsi wants to increase the volume of soda in their cans. They've decided to decrease the radius by 13 percent and decrease the height by 20 percent. They want to model the new volume based on the radius and height of the original cans. }
\end{center}
The solution is \( k = 1.90230 \), which is option C.\begin{enumerate}[label=\Alph*.]
\item \( k = 0.00338 \)
...
* This is the correct option and corresponds to the model: $V = \pi (0.87 r)^2 (0.80 h)$.
\item \( k = 0.60552 \)

This corresponds to the model: $V = (0.87 r)^2 (0.80 h)$.
\item \( \text{None of the above.} \)
...
\begin{center}
    \textit{ The rate of vibration of a string under constant tension varies based on the type of string and the length of the string. The rate of vibration of string $\omega$ decreases as the quartic length of the string increases. For example, when string $\omega$ is 4 mm long, the rate of vibration is 37 cm/s. }
\end{center}
The solution is \( k = 0.95 \), which is option A.\begin{enumerate}[label=\Alph*.]
\item \( k = 0.95 \)
...
\begin{tabular}{c|c|c|c|c|c|c|c|c|c}
\textbf{Year} &1 &2 &3 &4 &5 &6 &7 &8 &9\tabularnewline \hline
\textbf{Pop} &19970 &19940 &19910 &19880 &19850 &19820 &19790 &19760 &19730\end{tabular}The solution is \( \text{Linear} \), which is option A.\begin{enumerate}[label=\Alph*.]
\item \( \text{Linear} \)
...
\begin{center}
    \textit{ In an alternative galaxy, the quartic of the time, $T$ (Earth years), required for a planet to orbit Sun $\chi$ decreases as the quartic of the distance, $d$ (AUs), that the planet is from Sun $\chi$ decreases. For example, when Ea's average distance from Sun $\chi$ is 9, it takes 70 Earth days to complete an orbit. }
\end{center}
The solution is \( k = 3659.503 \), which is option C.\begin{enumerate}[label=\Alph*.]
\item \( k = 4.028 \)

This copies the constant used in the homework.
\item \( k = 157529610000.000 \)
...
\begin{center}
    \textit{ In an alternative galaxy, the cube of the time, $T$ (Earth years), required for a planet to orbit Sun $\chi$ increases as the quartic of the distance, $d$ (AUs), that the planet is from Sun $\chi$ increases. For example, when Ea's average distance from Sun $\chi$ is 9, it takes 55 Earth days to complete an orbit. }
\end{center}
The solution is \( k = 25.358 \), which is option B.\begin{enumerate}[label=\Alph*.]
\item \( k = 2.196 \)
